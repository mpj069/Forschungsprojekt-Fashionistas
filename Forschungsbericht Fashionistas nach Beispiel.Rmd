...
library(igraph)
el <- read.csv("https://raw.githubusercontent.com/cp071/Project-fashion-designer/main/muse%20Teilnetzwerk%20-%20Edgelist.csv", header=T, as.is=T, sep = ",")
...
ds <- delete_edge_attr(d, "position")
ds <- delete_edge_attr(d, "year")
list.edge.attributes(ds)

## [1] "position"

# Edge-Attribute zusammenfassen

is_simple(ds)

d_sim <- simplify(ds,edge.attr.comb=list(weight="sum"))
is_simple(d_sim)
```
...
IGRAPH 2c14a51 DN-B 76 63 -- 
+ attr: name (v/c), sex (v/n), sexuality (v/n), birthyear
| (v/n), scholar (v/n), transfer (v/n), main_workplace (v/c),
| value (v/n), type (v/n), position (e/x)
+ edges from 2c14a51 (vertex names):
 [1] Virgil Abloh       ->Lauryn Hill    
 [2] Annette de la Renta->Ode la Renta   
 [3] Giorgio Armani     ->Felize Kolibius
 [4] Christian Audigier ->Sarah Larson   
 [5] Christopher Bailey ->Cara Delevingne
 [6] Monica Belluci     ->DolceGabbana   
+ ... omitted several edges

# die aggregierten Werte für weight von d_sim

```{r}
list.edge.attributes(d_sim)
```
